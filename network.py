...
# Define a cost derivative function to calculate the error gradient. The error gradient is the direction (+/-) and slope of the cost function: the distance between the 
# output activation(s) and their target value(s) calculated using the mean squared error. The cost derivative simplifies to subtracting the network's output activation(s) 
# by its target value(s).
...
# defines the flow of data through the network. This reveals that the activations are the "behavior" of the network, and the parameters are the "knobs" we can turn to 
# tune that behavior. This also shows that the number of dimensions that the cost surface exists within corresponds directly to the number of parameters in the network. 
